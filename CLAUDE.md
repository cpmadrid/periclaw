...
```bash
./dev run                        # auto mode (mock if OPENCLAW_MOCK set, else ws)
./dev run --mode mock            # scripted fixture (offline)
...
1. **`Subscription`** (`app::App::subscription` ~line 2113) — picks the data-source stream:
   - `mock::connect` when `OPENCLAW_MOCK=1` — replays `assets/test/scenario_happy.json`.
...
- **`mock.rs`** — fixture-replay subscription used by `--mode mock`.
...
- **`office.rs`** — the canvas program. `running_rooms` (a HashSet keyed off either a Running job or an agent in `AgentStatus::Running`) drives whether each agent's sprite renders the **power-up sparkle** above it. *That HashSet is the answer to "why isn't the indicator showing?"* — usually because no signal flipped the agent's status to Running.
...
External Slack/Telegram/WhatsApp DM → agent
  ...
  → `lifecycle:start` → ActivityKind::Thinking → Running (sparkle on)
